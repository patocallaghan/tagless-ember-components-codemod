...
describe('findClassNameBindings()', () => {
  const TESTS = [
    ['', new Map()],
    ["classNameBindings: ['foo']", new Map([['foo', [null, null]]])],
    ["classNameBindings: ['fooBar']", new Map([['fooBar', [null, null]]])],
    ["classNameBindings: ['FOO']", new Map([['FOO', [null, null]]])],
...
describe('findEventHandlers()', () => {
  ...
  for (let [input, expected] of TESTS) {
    test(input || 'empty', () => {
      ...
      if (expected instanceof RegExp) {
        expect(() => findEventHandlers(props)).toThrow(expected);
      } else {
        expect(findEventHandlers(props)).toEqual(expected);
      }
    });
  }
});
