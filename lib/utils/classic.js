...
function findClassNameBindings(properties) {
  ...
  for (let binding of findStringArrayProperties(properties, 'classNameBindings')) {
    ...
    if (parts.length === 1) {
      classNameBindings.set(parts[0], [null, null]);
...
function findEventHandlers(properties) {
  return properties.value.map(n => n.key.name).filter(n => EVENT_HANDLER_METHODS.includes(n));
}
