...
module.exports = function transformClassicComponent(root, options) {
  ...
  // Add CODE MIGRATION HINT for `$` jQuery
  let jqueryPaths = j(objectArg).find(j.CallExpression, {
    callee: { name: '$' },
  });
  if (jqueryPaths.length !== 0 && usesJQuery) {
    jqueryPaths.forEach(path => {
      let comment = j.commentLine(
        ' CODE MIGRATION HINT: `jQuery` should not be used in Glimmer components. See TBC for more details on how to migrate.',
        true,
        false
      );
      while (path.parentPath.node.type !== 'Program') {
        ...
        if (['ExpressionStatement'].includes(path.node.type)) {
          ...
          let hasExistingAsyncComment = path.node.comments.find(c =>
            c.value.includes('`jQuery` should be used in Glimmer components.')
          );
  ...
  // Add CODE MIGRATION HINT for `this.elementId`
